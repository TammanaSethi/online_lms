      ...
      <div className='w-[100%] h-[70px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000047] z-10'>
        ...
        <div className='w-[30%] lg:flex items-center justify-center gap-4 hidden'>
          ...
          {!userData && <IoPersonCircleSharp className='w-[50px] h-[50px] fill-black cursor-pointer' onClick={()=>setShow(prev=>!prev)}/>}
            ...
            {userData?.photoUrl ? <img src={userData?.photoUrl} className='w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20] border-2 bg-black border-white cursor-pointer' onClick={()=>setShow(prev=>!prev)} />
            :<div className='w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20] border-2 bg-black border-white cursor-pointer' onClick={()=>setShow(prev=>!prev)}>
          ...
          <span className='px-[20px] py-[10px] bg-white text-black rounded-[10px] text-[18px] shadow-sm shadow-black font-light cursor-pointer ' onClick={handleLogout}>Logout</span>}

          {/* my profile & courses inside profile button */}

          { show && <div className='absolute top-[110%] right-[15%] flex items-center flex-col justify-center gap-2 text-[16px] rounded-md bg-[white] px-[15px] py-[10px] border-[2px] hover: text-white border-black hover:border-white cursor-pointer hover:bg-black'>
        ...
        <div className={`fixed top-0 left-0 w-[100vw] h-[100vh] bg-[#000000d6] flex items-center justify-center flex-col gap-5 z-10 lg:hidden ${showHam ? "translate-x-[0] transition duration-600" : "translate-x-[-100%] transition duration-600"}`}>
          ...
          <GiSplitCross className='w-[35px] h-[35px] fill-white absolute top-5 right-[4%]' onClick={()=>setShowHam(prev=>!prev)}/>
            ...
            {/* left slide bar options for phone */}

            {!userData && <IoPersonCircleSharp className='w-[50px] h-[50px] fill-black cursor-pointer' />}

              {userData?.photoUrl ? <img src={userData?.photoUrl} className='w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20] border-2 bg-black border-white cursor-pointer' />  
              :<div className='w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20] border-2 bg-black border-white cursor-pointer' >
                {userData?.name.slice(0,1).toUpperCase()}
              </div>}
