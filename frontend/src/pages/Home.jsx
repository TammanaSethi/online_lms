...
const Home = () => {
  ...
  const fetchPopularCourses = async () => {
    try {
      const res = await axios.get("http://localhost:8000/api/course/popular");
  ...
  return (
    <div className='w-[100%] overflow-hidden'>
      <div className='w-[100%] lg:h-[140vh] h-[70vh] relative '>
        ...
        <span className=' lg:text-[70px] text-[20px] absolute md:text-[40px] lg:top-[18%] top-[20%] w-[100%] flex items-center justify-center text-white font-bold'> Your Career Path</span>

        <div className='lg:top-[30%] absolute top-[75%] md:top[80%] w-[100%] flex items-center justify-center gap-3 flex-wrap'>
          ...
          <button className=' px-[20px] py-[10px] lg:bg-white bg-black lg:text-black text-white rounded-[10px] text-[18px] font-light flex gap-2 cursor-pointer items-center justify-center ' onClick={()=>navigate("/search")}>
            Search With AI 
            <img src={ai} className='w-[30px] h-[30px] rounded-full hidden lg:block'/> 
